<div x-data="{ {{ $id }}: {{ $open ?? false }} }"
    ...
    <div
        ...
        :class="{ 'text-zinc-900 bg-white border-zinc-200 dark:border-zinc-700 shadow-sm font-medium dark:bg-zinc-700/60 dark:text-zinc-100' : {{ $id }} == true, 'hover:bg-zinc-100 dark:hover:bg-zinc-700/60 hover:text-zinc-900 border-transparent dark:hover:text-zinc-100' : ({{ $id }} == false && !@json($active)) }"
    >
