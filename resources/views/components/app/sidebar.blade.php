<div x-data="{ sidebarOpen: false }" @open-sidebar.window="sidebarOpen = true"
    ...
    <div :class="{ '-translate-x-full': !sidebarOpen }"
         class="fixed top-0 left-0 flex items-stretch -translate-x-full overflow-hidden lg:translate-x-0 z-40 h-dvh md:h-screen transition-[width,transform] duration-150 ease-out bg-zinc-50 dark:bg-zinc-900 w-64 group @if(config('wave.dev_bar')){{ 'pb-10' }}@endif">
        <div class="flex flex-col justify-between w-full overflow-auto md:h-full h-svh pt-4 pb-2.5">
            <div class="relative flex flex-col">
                ...
                <div class="flex items-center px-5 space-x-2">
                    <a href="{{ route('home') }}"
                       class="flex justify-center items-center py-4 pl-0.5 space-x-1 font-bold text-zinc-900 text-xl dark:text-zinc-100"
                       wire:navigate>
                ...
                <div
                    ...
                    @if(auth()->user()->can(\App\Enums\Permissions::ManageEmployees->value) || auth()->user()->can(\App\Enums\Permissions::ManagePayrolls->value))
                        <x-app.sidebar-dropdown text="Karyawan" icon="phosphor-users-four-duotone" id="karyawan"
                            ...
                            @can(\App\Enums\Permissions::ManageEmployees->value)
                                <x-app.sidebar-link href="{{ route('karyawans.index') }}"
                                                    icon="phosphor-users-four-duotone"
                                                    :active="Request::is('karyawans')" wire:navigate>
                                    Karyawan
                                </x-app.sidebar-link>
                            @endcan
                            @can(\App\Enums\Permissions::ManagePayrolls->value)
                                <x-app.sidebar-link href="{{ route('payrolls.index') }}"
                                                    icon="phosphor-cash-register-duotone"
            ...
            <div class="relative px-2.5 space-y-1.5 text-zinc-700 dark:text-zinc-400">
                ...
                <x-app.sidebar-link href="#" target="_blank" icon="phosphor-book-bookmark-duotone" active="false">
                    Documentation
                </x-app.sidebar-link>
                <x-app.sidebar-link href="#" target="_blank" icon="phosphor-chat-duotone" active="false">Questions
                </x-app.sidebar-link>
                <x-app.sidebar-link href="#" icon="phosphor-book-open-text-duotone"
                                    :active="Request::is('changelog') || Request::is('changelog/*')">Changelog
                </x-app.sidebar-link>
                ...
                <div x-show="sidebarTip" x-data="{ sidebarTip: $persist(true) }" class="px-1 py-3" x-collapse x-cloak>
                    <div
                        ...
                        <h5 class="pb-1 text-sm font-bold -translate-y-0.5">HRIS App</h5>
                        <p class="block pb-1 text-xs opacity-80 text-balance">Access the full-fledge HRIS app here only on HRIS <Folt></Folt>.</p>
                    </div>
                </div>
